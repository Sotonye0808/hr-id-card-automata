...
- **Template Library** — Save named templates to localStorage, load, rename, delete, export as JSON, and import from JSON files. **Tap an item to select it** — **Save** overwrites the selected template, while **"Save As New"** creates a template copy with a unique ID for true multi-template management.
...
4. **Save Template** — Click the library icon to save your design. Tap a template in the list to select it, then **Save** to overwrite that template, or use **Save As New** to keep multiple template versions. Export as JSON for sharing. Toast notifications confirm saves.
